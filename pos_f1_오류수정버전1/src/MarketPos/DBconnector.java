...
public class DBconnector {
   ...
   public static DBconnector getInstacne() {
	   if(dbconnector == null) {
   ...
   public ArrayList pdisplay() {
      ...
      try {
         ...
         while (rs.next()) { //다음 방문할 값이 존재하는 동안 루프
            ...
            data.add(barcode+"  "+pname+"  "+ptype+"  "+pamount+"  "+expiration+" "+price+"\r\n");
         }
         rs.close();
         stmt.close();
     ...
     // System.out.println("이름   전화번호   회원등급   적립금   나이");
      try {
         ...
         while (rs.next()) { //다음 방문할 값이 존재하는 동안 루프
            ...
            data.add(cname+"\r\t"+phone+"\r\t"+grade+"\r\t"+saving+"\r\t"+age+"\r\n");
   ...
   public ArrayList bdisplay() {
      ...
      try {
         ...
         while(rs.next()) {                        
            ...
            int price = rs.getInt("price");               
            data.add(barcode+"\r\t"+amount+"\r\t"+pname+"\r\t"+method+"\r\t"+price+"\r\t");
